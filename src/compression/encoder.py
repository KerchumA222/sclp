...
def encode_palette(clipped_weights_bf16: np.ndarray) -> dict:
    ...
    Output format matches the HIP GPU encoder:
      ...
      sm_stream:      uint8[N]          — one byte per weight: sign(7) | mantissa(6:0)
    ...
    # 5. SM stream: one byte per weight — sign(7) | mantissa(6:0)
    sign     = ((weights >> 15) & 0x1).astype(np.uint8)
    mantissa = (weights & 0x7F).astype(np.uint8)
    sm_stream = ((sign << 7) | mantissa).astype(np.uint8)
