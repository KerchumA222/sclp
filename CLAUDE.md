...
Applied to **sidecar selection, not palette k-means** (naive k-means weighting regressed PPL 5×; but note palette *histogram* weighting by Σ|w| is a separate, validated win — review item #4). Two sidecar tiers: (1) mandatory `dist > sidecar_dist`, (2) discretionary top-`budget` fraction by `importance × reconstruction_err²` (importance = `imatrix_value[flat_index % K]`; the `err²` ranking replaced the older `× distance` in review item #1 — dist-0 weights with mantissa-grid error are now eligible).
...
```
Budget sweep on Gemma4 mixed: 0→13,909 PPL, 0.5%→1,506, **1%→940**, 2%→1,026 (2% within error of 1% — quality floor). **Gate the budget on KL-divergence vs full precision, NOT perplexity** — PPL (in- *and* out-of-distribution) is confounded and noisy. On Gemma4-12B-IT-QAT, OOD PPL across budgets was non-monotonic noise (0.5% scored *worst*), but KL vs a Q8_0 reference is cleanly monotone-decreasing (budget 0→2%: mean KLD 1.46→1.42→1.19→1.01), confirming the nested sidecar always moves closer to truth — there is no "knee," only a size↔fidelity tradeoff, so 1% remains a sensible default (KL argues for higher, never lower). Use a Q8_0 proxy reference when BF16 won't fit the card (all models vs the same ref → ranking exact). See `experimental_results.md` §18 (and §18a: the #4 palette change *raised* in-distribution wikitext PPL 8.95→9.91 while *lowering* KL 9.8× — better reconstruction de-regularizes in-distribution text; KL is the decider). **Calibration source:** prefer BF16 over Q5_K_M to avoid the quant's own rounding noise contaminating the importance signal (standard practice; not isolated in a controlled A/B here). The Gemma4 sweep above used a **Q5_K_M-sourced** imatrix — Gemma4 BF16 is 48 GB, won't fit the 24 GB card. Where a published BF16 imatrix existed off the shelf we used it (Qwen3.6: `eval_data/qwen36-bartowski-imatrix.gguf`, bartowski calibration_datav5, BF16-source). So "~hundreds PPL" contamination cost is inherited wisdom, not a number measured in this project.
...
tg (Llama-3-8B, RX 7900 XTX, all coherent): SCLP4 26.8 · SCLP5 28.9 · **SCLP6 34.2 (fused GEMV RE-ENABLED)** · SCLP8 40.6 (llama-bench tg128, sidecar v2). NB: SCLP8 was ~30 with the old binary-search sidecar fold; sidecar v2's O(1) row-range lookup recovered it to 40.6 (folded) vs 43.9 no-fold — the fold now costs ~7%, not the 2× the "66→30" regression implied. The 66 figure was pre-sidecar and is not reproducible via llama-bench. SCLP4/5/6 numbers predate v2 and may be similarly higher.
...
**SCLP4M is the recommended 4-bit mode (measured, 2026-06-16 — experimental_results §19).** KL vs higher-precision ref: dense Llama-3-8B whole-model 4-bit **mean KLD 0.057 vs SCLP4's 0.557 (9.7× closer, near-lossless PPL 1.02×) for +1.6% size**; MoE Gemma4-26B MIXED (only gate/up swapped) **0.353 vs 0.434 (−19%)**, coherent, +0.73 GiB. Direction holds on both classes (decisive dense, incremental when confined to bulk gate/up). The KL/PPL inversion recurs (SCLP4M: higher wiki PPL, lower KL — KL decides). SCLP4M also doesn't mode-collapse *pure* where SCLP4 does on IT models. **Don't propagate the codebook to 6/8-bit by default:** per-block codebook costs `2^idx × 2 B`/256-block → SCLP4M +0.5 b/wt (cheap), "SCLP6M" +2.0 b/wt, "SCLP8M" +8 b/wt (doubles storage) — it's a 4-bit-native trick; "SCLP6M vs SCLP6" is untested.
